		...
		noteContentLv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if (noteBookEntity.getImgUrl().size() > 0) {
					...
					Toast.makeText(context, "图片没锟斤拷锟较达拷锟缴癸拷", Toast.LENGTH_LONG)
							.show();
				}
	...
	public void onClick(View v) {
		...
		case R.id.sure:
			if (btnSure.getText().equals(
				...
				if (StringUtil.isEmpty(etNoteTitle.getText().toString())) {
					Toast.makeText(this, "锟斤拷锟解不锟斤拷为锟斤拷", Toast.LENGTH_SHORT).show();
					etNoteTitle.requestFocus();
					return;
				}
				if (StringUtil.isEmpty(etNoteContent.getText().toString())) {
					Toast.makeText(this, "锟斤拷锟捷诧拷锟斤拷为锟斤拷", Toast.LENGTH_SHORT).show();
		...
		case R.id.note_share:
			...
			try {
				...
				Toast.makeText(this, "锟斤拷锟斤拷图片锟斤拷锟斤拷", Toast.LENGTH_SHORT).show();
				return;
			}
			intent.setType("image/*");
			intent.putExtra(Intent.EXTRA_SUBJECT, "Share");
			// 锟斤拷锟斤拷锟斤拷锟�			intent.putExtra(Intent.EXTRA_TEXT, "喜锟斤拷锟斤拷");
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode != Activity.RESULT_OK) {// result is not correct
			...
			case Constants.USER_STATUS.PHOTO_PICKED_WITH_DATA:
				...
				try {
//					// 使锟斤拷ContentProvider通锟斤拷URI锟斤拷取原始图片
...
//					}
			...
			case Constants.USER_STATUS.CAMERA_WITH_DATA:
				try {
					...
					// 锟斤拷锟斤拷锟斤拷锟斤拷图片锟斤拷示锟节斤拷锟斤拷锟较ｏ拷锟斤拷锟斤拷锟芥到锟斤拷锟斤拷
	...
	public void onRefresh() {
		if (DeviceUtil.is3GorWifi()) {
			...
			Toast.makeText(context, "锟斤拷前锟斤拷锟界不锟斤拷wifi", Toast.LENGTH_LONG).show();
			noteContentLv.onRefreshComplete();
		}
	}
